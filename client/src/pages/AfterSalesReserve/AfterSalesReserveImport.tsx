...
import { importAfterSalesReserve } from '@client/src/api/data-import';
import type { ImportResult } from '@client/src/api/data-import';
import CanEdit from '@client/src/components/CanEdit';

const TEMPLATE_HEADERS = ['客户简称', '售后准备金率'];

interface AfterSalesReserveImportProps {
  open: boolean;
...
function downloadTemplate() {
  ...
  XLSX.utils.book_append_sheet(wb, ws, '售后准备金率');
  XLSX.writeFile(wb, '售后准备金率导入模板.xlsx');
}
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const rateIdx = headerRow.indexOf('售后准备金率');

        if (nameIdx < 0 || rateIdx < 0) {
          reject(new Error('模板缺少必要列：客户简称、售后准备金率'));
          return;
        }
...
export default function AfterSalesReserveImport({
  open,
  onOpenChange,
  onSuccess,
}: AfterSalesReserveImportProps) {
  ...
  const handleImportFile = async (
    ...
    try {
      ...
      const result = await importAfterSalesReserve(importFile);
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="rounded-sm max-w-lg">
          <DialogHeader>
            <DialogTitle>批量导入售后准备金率</DialogTitle>
            <DialogDescription>

...
import {
  getExcessMarketingList,
  createExcessMarketing,
  updateExcessMarketing,
  deleteExcessMarketing,
} from '@client/src/api/excess-marketing';
import type { ExcessMarketingItem } from '@shared/excess-marketing';
import CanEdit from '@client/src/components/CanEdit';
import ExcessMarketingImport from './ExcessMarketingImport';
...
export default function ExcessMarketingPage() {
  const [items, setItems] = useState<ExcessMarketingItem[]>([]);
  ...
  const [editingItem, setEditingItem] =
    useState<ExcessMarketingItem | null>(null);
  ...
  const fetchList = useCallback(async () => {
    setLoading(true);
    try {
      const result = await getExcessMarketingList({
  ...
  const openEditDialog = (item: ExcessMarketingItem) => {
    setEditingItem(item);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingItem) {
        await updateExcessMarketing(editingItem.id, payload);
        toast.success('更新成功');
      } else {
        await createExcessMarketing(payload);
        toast.success('创建成功');
      }
      setDialogOpen(false);
      fetchList();
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteExcessMarketing(deleteTarget.id);
      toast.success('删除成功');
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        {loading ? (
          ...
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {items.map((item: ExcessMarketingItem) => (
                <tr
                  key={item.id}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="rounded-sm max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editingItem ? '编辑超额营销费用率' : '新增超额营销费用率'}
            </DialogTitle>
            <DialogDescription>
              {editingItem
                ? '修改超额营销费用率信息'

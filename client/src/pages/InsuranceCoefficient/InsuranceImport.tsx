...
import { importInsuranceCoefficient } from '@client/src/api/insurance';
import type { ImportInsuranceResponse } from '@shared/insurance';

const TEMPLATE_HEADERS = ['信用条件', '保费系数(%)'];

interface InsuranceImportProps {
  open: boolean;
...
function downloadTemplate() {
  ...
  XLSX.utils.book_append_sheet(wb, ws, '保费系数');
  XLSX.writeFile(wb, '保费系数导入模板.xlsx');
}

function parseExcelFile(
  file: File,
): Promise<Array<{ creditCondition: string; coefficient: string }>> {
    ...
    reader.onload = (e) => {
      try {
        ...
        const creditIdx = headerRow.findIndex(
          (h: string) => h.includes('信用条件') || h.includes('信用'),
        );
        const coeffIdx = headerRow.findIndex(
          (h: string) =>
            h.includes('系数') || h.includes('保费') || h.includes('coefficient'),
        );

        if (creditIdx < 0 || coeffIdx < 0) {
          reject(new Error('模板缺少必要列：信用条件、保费系数(%)'));
          return;
        }

        const items = rows.slice(1).map((row: string[]) => ({
          creditCondition: String(row[creditIdx] ?? '').trim(),
          coefficient: String(row[coeffIdx] ?? '').trim(),
        }));
...
export default function InsuranceImport({
  ...
  onOpenChange,
  onSuccess,
}: InsuranceImportProps) {
  const [importing, setImporting] = useState(false);
  const [importResult, setImportResult] =
    useState<ImportInsuranceResponse | null>(null);
  ...
  const handleImportFile = async (
    ...
    try {
      ...
      const result = await importInsuranceCoefficient(parsedItems);
      setImportResult(result);
      if (result.imported > 0 || result.updated > 0) {
        toast.success(
          `导入完成：新增 ${result.imported} 条，更新 ${result.updated} 条`,
        );
        onSuccess();
      }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="rounded-sm max-w-lg">
        <DialogHeader>
          <DialogTitle>批量导入保费系数</DialogTitle>
          <DialogDescription>
            请选择 Excel 文件（.xlsx），已存在的信用条件将被更新
          </DialogDescription>
        </DialogHeader>
                  ...
                  <table className="w-full text-xs border-collapse">
                    ...
                    <tbody>
                      {importResult.errors.map(
                        (err: { row: number; message: string }, idx: number) => (
                          <tr
                            key={idx}

import {
  Controller,
  Get,
  Post,
  Put,
  Delete,
  Body,
  Param,
  Query,
} from '@nestjs/common';
import { NeedLogin } from '@server/lib/platform';
import { UseInterceptors } from '@nestjs/common';
import { JwtRoleInterceptor } from '@server/common/guards/jwt-role.interceptor';
import { PriceSensitivityService } from './price-sensitivity.service';

@Controller('api/price-sensitivities')
@UseInterceptors(JwtRoleInterceptor)
export class PriceSensitivityController {
  constructor(private readonly service: PriceSensitivityService) {}

  @Get()
  async findAll(
    @Query('region') region?: string,
    @Query('page') page?: string,
    @Query('pageSize') pageSize?: string,
  ) {
    const result = await this.service.findAll({
      region: region || undefined,
      page: page ? parseInt(page, 10) : 1,
      pageSize: pageSize ? parseInt(pageSize, 10) : 20,
    });
    return { code: 0, data: result };
  }

  @NeedLogin()
  @Post()
  async create(
    @Body() body: { region: string; channelType: string; discount: number },
  ) {
    const result = await this.service.create(body);
    return { code: 0, data: result };
  }

  @NeedLogin()
  @Post('import')
  async importItems(
    @Body() body: { items: Array<{ region: string; channelType: string; discount: string }> },
  ) {
    const result = await this.service.importItems(body.items);
    return { code: 0, data: result };
  }

  @NeedLogin()
  @Put(':id')
  async update(
    @Param('id') id: string,
    @Body() body: { region: string; channelType: string; discount: number },
  ) {
    const result = await this.service.update(id, body);
    return { code: 0, data: result };
  }

  @NeedLogin()
  @Delete(':id')
  async remove(@Param('id') id: string) {
    const result = await this.service.remove(id);
    return { code: 0, data: result };
  }
}
